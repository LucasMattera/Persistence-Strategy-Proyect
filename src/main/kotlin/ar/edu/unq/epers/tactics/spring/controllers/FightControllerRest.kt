...
class FightControllerRest(private val peleaService: PeleaService,val aventureroService: AventureroService) {
    ...
    fun receiveAbility(@PathVariable fightId: Long, @RequestBody request:ReceiveAbilityRequest):AventureroDTO {
        val habilidad = request.ability.aModelo()
        habilidad.objetivo = aventureroService.recuperar(request.adventurerId)
        val aventurero = peleaService.recibirHabilidad(fightId,request.adventurerId,habilidad)
